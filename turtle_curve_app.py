...
class TurtleCurveApp:
    def __init__(self):
        ...
        self.coefficients = [
            random.uniform(-0.001, 0.001) for _ in range(self.degree + 1)
        ]
        ...
        self.screen.onkey(self.augment_step_size, "Up")
        self.screen.onkey(self.reduce_step_size, "Down")
    ...
    def augment_step_size(self):
        """Increase the step size for turtle movement"""
        self.step_size += 10
        print(f"Step size increased to {self.step_size}")

    def reduce_step_size(self):
        """Decrease the step size for turtle movement"""
        if self.step_size > 1:
            self.step_size -= 10
